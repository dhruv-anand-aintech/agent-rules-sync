...
def install_linux():
    ...
    # Detect Termux — no systemd available
    if os.path.isdir("/data/data/com.termux/files/usr") and "TERMUX_VERSION" in os.environ:
        return install_termux()
...
def install_termux():
    """Install on Termux/Android using nohup + ~/.bashrc auto-restart."""
    ...
    start_script = config_dir / "start-daemon.sh"
    start_script.write_text(
        f"#!/data/data/com.termux/files/usr/bin/sh\n"
        f"# Auto-generated by agent-rules-sync installer\n"
        f"cd {Path.home()}\n"
        f"nohup {python_path} -m agent_rules_sync watch >> {log_dir}/daemon.log 2>&1 &\n"
        f"echo $! > {log_dir}/daemon.pid\n",
        encoding="utf-8",
    )
    ...
    marker = "# agent-rules-sync daemon autostart"
    entry = (
        f"\n{marker}\n"
        f"if [ -f {start_script} ] && [ -z \"$AGENT_SYNC_DAEMON_SKIP\" ]; then\n"
        f"  bash {start_script} >/dev/null 2>&1\n"
        f"fi\n"
    )
    if bashrc.exists():
        ...
        bashrc.write_text(entry, encoding="utf-8")
    ...
    try:
        ...
        print("✓ Termux daemon installed and started")
        print(f"  Start script: {start_script}")
        print(f"  Logs: {log_dir}/daemon.log")
        print(f"  Auto-starts on new shell sessions via ~/.bashrc")
        print(f"  To disable auto-start: export AGENT_SYNC_DAEMON_SKIP=1")
        return True
    except subprocess.CalledProcessError as e:
        print(f"⚠️  Could not start Termux daemon: {e}")
        print(f"   Try manually: bash {start_script}")
        return False
